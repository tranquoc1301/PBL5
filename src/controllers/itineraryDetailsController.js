...
exports.getDetailsByUserAndItinerary = async (req, res, next) => {
  ...
  try {
    const details = await ItineraryDetailService.getDetailsByUserAndItinerary(
      userId,
      itineraryId
    );
...
exports.createItineraryDetail = async (req, res, next) => {
  try {
    const newItineraryDetail =
      await ItineraryDetailService.createItineraryDetail(req.body);
...
exports.deleteItineraryDetailsByItineraryId = async (req, res, next) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({
        message: `Deleted all itinerary details for itinerary_id ${itinerary_id}`,
      });
  } catch (error) {
    console.log("Error ne: ", error.errors || error);
    next(error);
